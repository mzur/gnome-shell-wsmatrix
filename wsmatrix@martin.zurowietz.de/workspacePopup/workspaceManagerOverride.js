...
export default class WorkspaceManagerOverride {
    ...
    _takeOverSwipeGestures() {
        // The original WorkspaceAnimationController still has its own (horizontal)
        // swipe tracker attached to global.stage. Merely disabling it isn't
        // durable — that controller's own overview 'hiding' handler re-enables it
        // (`this._swipeTracker.enabled = true`) — so it would keep stealing
        // horizontal swipes from our replacement controller. Destroy its gesture
        // outright (we recreate it on disable). The destroyed tracker object is
        // left in place so the stale overview handlers toggling its `enabled`
    ...
    _restoreStockWorkspaceSwipe() {
        ...
        // Recreate the gesture GNOME's WorkspaceAnimationController set up in its
        // constructor, rewired to the original controller's own handlers.
    ...
    _handleSwipeOverrideChanged() {
        ...
        if (enabled) {
            // We take over vertical swipes, so silence the overview's
            // swipe-to-open gesture (the overview is still reachable via the
            // keyboard shortcut).
            ...
            // Show the switcher popup after a swipe, mirroring keyboard
            // navigation but without a modal grab (see _showWorkspaceSwitcherPopup).
    ...
    _restoreSwipeGestures() {
        // Tear down our controller's trackers and restore the gestures we
        // replaced when taking over.
    ...
    _showWorkspaceSwitcherPopup(toggle, swipe = false) {
        ...
        this.monitors.forEach((monitor) => {
            ...
            if (!this.wm._wsPopupList[monitorIndex]) {
                ...
                // Existing popup: refresh it. For swipes, also move the highlight
                // to the new active workspace so it follows rapid swipes.
                if (swipe) {
